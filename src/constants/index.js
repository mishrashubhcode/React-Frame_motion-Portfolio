import project1 from "../assets/projects/project-1.jpg";
import project2 from "../assets/projects/project-2.jpg";
import project3 from "../assets/projects/project-3.jpg";
import project4 from "../assets/projects/project-4.jpg";

export const HERO_CONTENT = `Every line of code tells a story—mine speaks of €1.5M revenue saved, 18% user retention boosts, and 40% productivity gains. Full-Stack Developer with 3+ years across TCS, Infosys, and Accenture, specializing in React, Node.js, C#, and AWS/Azure. Expert in AI-enhanced development workflows that transform business challenges into scalable digital solutions.`;

export const ABOUT_TEXT = `⚡ I don't wait for tomorrow to build applications—I leverage AI to transform ideas into reality today.

💻 Full-Stack Developer with 3+ years delivering scalable web applications across industry leaders like TCS, Infosys, and Accenture.

🔧 Expert in React, Node.js, C#, and AWS/Azure with AI-enhanced development workflows that boost productivity by 40%.

🚀 Proven Impact:
• €1.5M revenue loss prevention through critical system optimization
• 18% user retention increase across 200,000+ active users  
• 99% system uptime with 75% manual effort reduction
• 95% code quality maintained while achieving AI-powered productivity gains

🌐 My technical expertise spans the complete development lifecycle—from crafting intuitive React frontends to building robust Node.js backends, optimizing cloud infrastructure on AWS/Azure, and integrating AI tools like ChatGPT and GitHub Copilot for enhanced development velocity.

🎓 Recently strengthened my skills with a Post Graduate Certificate from Humber College, Toronto, where I mastered modern full-stack development practices. I thrive in agile environments, excel at translating complex business requirements into elegant technical solutions, and continuously explore emerging technologies to stay ahead of industry trends.

🚀 Ready to bring innovative ideas to life through code, collaboration, and cutting-edge technology.`;


export const EDUCATION = [
  {
    year: "Jan. 2023 - Dec. 2023",
    degree: "Post Graduate Certificate",
    college: "Humber College",
    course: `Web Development`,
    technologies: ["JavaScript", "React.js", "Node.js", "MongoDB", "ASP.NET MVC", "Cypress"],
  },
  {
    year: "Aug. 2016 - Oct. 2020",
    degree: "B.Tech",
    college: "Maharaja Agrasen Institute of Technology",
    course: `Electronics and Communication Engineering`,
    technologies: ["Java", "Python", "OOPs", "Computer Networking", "OS"],
  },
];

export const EXPERIENCES = [
  {
    year: "April 2025 - Present",
    role: "System Engineer (Front-End Developer)",
    company: "Tata Consultancy Services",
    achievements: [
      "Boosted development productivity by 40% using AI tools like ChatGPT and GitHub Copilot, reducing debugging time from 2 hours to 45 minutes",
      "Maintained 95%+ code quality standards while implementing AI-enhanced workflows across multiple projects",
      "Delivered 18% user retention increase through enhanced React component architecture for 200,000+ active users",
      "Authored comprehensive technical documentation for 15+ sprint deliveries, driving continuous agile enhancements",
      "Collaborated with cross-functional teams to translate complex business requirements into scalable frontend solutions"
    ],
    technologies: ["React.js", "JavaScript", "Node.js", "C#", "GitHub Copilot", "ChatGPT", "AWS", "Azure"],
  },
  {
    year: "April 2024 - March 2025",
    role: "Software Developer",
    company: "Infosys Ltd.",
    achievements: [
      "Spearheaded development of 4+ merchandise and inventory-based web applications using ASP.NET and MERN stack",
      "Enhanced system performance by 20% through optimized database queries and improved application architecture",
      "Streamlined system workflows by 30% via AWS RDS Aurora database enhancements and Lambda-integrated SNS/SQS notifications",
      "Facilitated collaborative brainstorming sessions with 10-member teams to translate functional and business requirements",
      "Revamped Visual Basic applications, contributing to 15% boost in user productivity and system stability",
      "Accelerated feature delivery by 35% using AI-powered tools for intelligent code generation and optimization"
    ],
    technologies: ["C#", "ASP.NET", "React.js", "Node.js", "MongoDB", "MySQL", "Snowflake", "AWS", "ChatGPT", "Claude"],
  },
  {
    year: "Oct. 2023 - Dec. 2023",
    role: "Software Developer Intern",
    company: "RadicalX",
    achievements: [
      "Enhanced platform performance by 10% and user engagement through comprehensive UI refresh",
      "Implemented new features using Node.js, React, and MongoDB for improved user experience",
      "Played key role in launching RadicalX Missions feature, resulting in increased platform engagement",
      "Spearheaded development of subscription page, driving 20% growth in user adoption and subscriptions",
      "Collaborated with design and product teams to implement responsive web components"
    ],
    technologies: ["HTML", "CSS", "Vue.js", "MongoDB", "React.js", "Node.js", "JavaScript"],
  },
  {
    year: "Feb. 2021 - Dec. 2022",
    role: "Application Development Associate",
    company: "Accenture",
    achievements: [
      "Managed overhaul of 2 crucial ASP.NET based CRM applications, refining functionality to address shifting business demands",
      "Automated report generation processes, cutting manual effort by 75% and reducing creation time from 1 hour to 15 minutes",
      "Developed Console application (CLR) for component-level repairs, reducing end-user wait time by 10 hours/month",
      "Eliminated critical production bugs, saving 100+ hours/month and ensuring 99% system uptime",
      "Led migration of VB6 and INRIS applications to Azure Cloud, improving scalability and cutting costs by 15%",
      "Rectified 95% of issues during SIT and UAT phases, ensuring smooth deployment with <2% post-launch defects",
      "Remedied critical SSIS Package vulnerability, protecting vital business data and preventing €1.5M revenue loss",
      "Optimized SQL queries and developed efficient stored procedures, boosting data retrieval speed by 20%",
      "Led successful rebranding of OTIS application, achieving 100% brand consistency across 50+ modules",
      "Documented 100+ JIRA requirements including design documents and implementation plans, improving execution by 30%",
      "Worked extensively on ServiceNow and JIRA tickets with 95% first-call resolution rate"
    ],
    technologies: ["Python", "ASP.NET", "C#", "MySQL", "SSIS", "Postman", "PuTTy", "Azure", "Oracle", "Visual Studio"],
  },
];


export const PROJECTS = [
  {
    title: "E-Commerce Website",
    image: project4,
    description: "A comprehensive full-stack e-commerce platform built to handle high traffic and provide seamless shopping experience. Designed with modern web technologies to support scalable business operations.",
    features: [
      "User authentication and authorization with secure session management",
      "Product catalog with advanced search and filtering capabilities",
      "Shopping cart functionality with persistent state management",
      "Order processing and payment integration",
      "Admin dashboard for inventory and order management",
      "Responsive design optimized for mobile and desktop"
    ],
    implementation: "Built using Node.js and Express.js for robust backend API development, with MongoDB for flexible data storage. Implemented Pug templating engine for server-side rendering, ensuring fast page loads and SEO optimization. Used advanced database indexing and caching strategies to support 100+ concurrent users.",
    impact: "Successfully handles over 100 concurrent users with 99.5% uptime. Reduced page load times by 40% through optimized database queries and efficient caching mechanisms. Achieved 95% customer satisfaction rate based on user feedback.",
    technologies: ["HTML", "CSS", "Pug", "Node.js", "MongoDB", "Express.js"],
    githubUrl: "https://github.com/mishrashubhcode/E-Commerce-Website",
    // liveUrl: "https://your-ecommerce-demo.com"
  },
  {
    title: "Furry Friend Finder",
    image: project1,
    description: "A comprehensive web platform designed to connect pet enthusiasts, facilitate pet adoptions, and build a community of animal lovers. Features secure user management and real-time communication.",
    features: [
      "User registration and profile management with photo uploads",
      "Pet listing system with detailed profiles and image galleries",
      "Advanced search filters by location, breed, age, and characteristics",
      "Real-time messaging system between pet owners and adopters",
      "Favorites system to save preferred pets",
      "Admin panel for moderation and platform management"
    ],
    implementation: "Developed using React.js for dynamic user interfaces and Vue.js for specific component interactions. Backend powered by Node.js and Express.js with MongoDB for data persistence. Implemented JWT authentication for secure user sessions and bcrypt for password hashing.",
    impact: "Successfully connected over 500 pet enthusiasts with 80+ successful adoptions facilitated. Reduced average time to find pets by 60% through advanced search algorithms. Maintained 98% user retention rate with positive community feedback.",
    technologies: ["React.js", "Vue.js", "Node.js", "Express.js", "MongoDB", "JWT"],
    githubUrl: "https://github.com/mishrashubhcode/Furry-Friend-Finder",
    // liveUrl: "https://furry-friend-finder-demo.com"
  },
  {
    title: "News Aggregator",
    image: project3,
    description: "A dynamic news aggregation platform that collects, processes, and displays real-time news from multiple sources. Built to provide users with comprehensive news coverage in a clean, organized interface.",
    features: [
      "Real-time news fetching from multiple API sources",
      "Category-based news organization (Technology, Sports, Politics, etc.)",
      "Search functionality with keyword highlighting",
      "Responsive grid layout for optimal reading experience",
      "News source credibility indicators",
      "Social sharing integration for articles"
    ],
    implementation: "Built with Node.js backend utilizing multiple REST APIs including NewsAPI, Guardian API, and BBC News API. Implemented automated data cleaning and duplicate removal algorithms. Used caching mechanisms to optimize API calls and reduce loading times.",
    impact: "Aggregates and displays 200+ news articles daily from 15+ reliable sources. Achieved 90% user engagement rate with average session time of 8 minutes. Reduced news discovery time by 70% through intelligent categorization.",
    technologies: ["HTML", "CSS", "Node.js", "REST APIs", "NewsAPI", "Guardian API"],
    githubUrl: "https://github.com/mishrashubhcode/News-Aggregator",
    // liveUrl: "https://news-aggregator-demo.com"
  },
  {
    title: "SuperAI DuckDB - NLP to SQL Converter",
    image: project2,
    description: "An intelligent full-stack application that converts natural English language queries into SQL commands using advanced NLP techniques. Built to democratize data analysis by allowing non-technical users to interact with databases using plain English.",
    features: [
      "Natural Language Processing engine for query interpretation",
      "Real-time English-to-SQL conversion with high accuracy",
      "CSV file parsing and dynamic database schema generation",
      "Interactive web interface for seamless user experience",
      "DuckDB integration for high-performance analytical queries",
      "Error handling and query validation systems"
    ],
    implementation: "Frontend built with modern React.js for responsive user interactions, while the backend leverages Python with advanced NLP libraries including spaCy and transformers. Integrated DuckDB for lightning-fast analytical processing of large datasets. Implemented custom parsing algorithms for CSV ingestion and dynamic schema creation.",
    impact: "Reduced data analysis time by 80% for non-technical users. Successfully processed 10,000+ natural language queries with 92% accuracy rate. Enabled business stakeholders to perform complex data analysis without SQL knowledge, increasing data-driven decision making by 60%.",
    technologies: ["React.js", "Python", "NLP", "spaCy", "DuckDB", "Natural Language Processing", "SQL", "CSV Processing"],
    githubUrl: "https://github.com/mishrashubhcode/SuperAI_DuckDB_Frontend",
    backendUrl: "https://github.com/mishrashubhcode/SuperAI_DuckDB_Backend",
    // liveUrl: "https://superai-duckdb-demo.com"
  },
];


export const CONTACT = {
  phoneNo: "+91-8076288289 ",
  email: "shubhmish1997@gmail.com",
};
